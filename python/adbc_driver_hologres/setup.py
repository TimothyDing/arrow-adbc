...
library = os.environ.get("ADBC_HOLOGRES_LIBRARY")
target = source_root.joinpath(
    "./adbc_driver_hologres/libadbc_driver_hologres.so"
).resolve()
if not library:
    if os.environ.get("_ADBC_IS_SDIST", "").strip().lower() in ("1", "true"):
        print("Building sdist, not requiring ADBC_HOLOGRES_LIBRARY")
    elif os.environ.get("_ADBC_IS_CONDA", "").strip().lower() in ("1", "true"):
        print("Building Conda package, not requiring ADBC_HOLOGRES_LIBRARY")
    elif target.is_file():
        print("Driver already exists (but may be stale?), continuing")
    else:
        raise ValueError("Must provide ADBC_HOLOGRES_LIBRARY")
else:
...
version = get_version("adbc_driver_hologres")
