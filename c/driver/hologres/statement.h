...
enum class HologresIngestMethod {
  kCopy,   // Standard PostgreSQL COPY (default)
  kStage,  // Hologres internal stage-based ingestion
};
...
class TupleReader final : public std::enable_shared_from_this<TupleReader> {
 ...
 private:
  ...
  std::unique_ptr<adbcpq::PostgresCopyStreamReader> copy_reader_;
  int64_t row_id_;
  int64_t batch_size_hint_bytes_;
  bool is_finished_;
};

/// Hologres statement (query execution and bulk ingestion).
class HologresStatement {
 public:
  HologresStatement()
      : connection_(nullptr),
        query_(),
 ...
 private:
  ...
  std::shared_ptr<HologresConnection> connection_;
  ...
  struct {
    ...
    OnConflictMode on_conflict = OnConflictMode::kNone;
    HologresIngestMethod hologres_ingest_method = HologresIngestMethod::kCopy;
  } ingest_;
